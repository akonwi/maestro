...
export function useQuickImport() {
	...
	const canQuickImport = useCallback((): {
		canImport: boolean;
		reason?: string;
	} => {
		...
		if (
			!config.importPreferences?.leagueMappings ||
			Object.keys(config.importPreferences.leagueMappings).length === 0
		) {
		...
		const season =
			config.importPreferences?.selectedSeason || new Date().getFullYear();
		let hasAnyMappings = false;

		for (const apiLeagueId of Object.keys(
			config.importPreferences.leagueMappings,
		).map(Number)) {
	...
	const quickImport = useCallback(async (): Promise<QuickImportResult> => {
		...
		try {
			...
			for (const [apiLeagueIdStr, targetLocalLeagueId] of Object.entries(
				leagueMappings,
			)) {
				const apiLeagueId = Number(apiLeagueIdStr);

				const mappings = teamMappingService.getTeamMappings(
					apiLeagueId,
					season,
				);
				...
				try {
					...
					const completedResult = await matchImportService.importMatches(
						apiLeagueId,
						season,
						targetLocalLeagueId,
					);

					totalImported += completedResult.imported;
					totalSkipped += completedResult.skipped;
					allErrors.push(...completedResult.errors);

					// Import upcoming matches (next 10 matches)
					const upcomingResult = await matchImportService.importUpcomingMatches(
						apiLeagueId,
						season,
						targetLocalLeagueId,
						15,
					...
					totalSkipped += upcomingResult.skipped;
					allErrors.push(...upcomingResult.errors);
				} catch (error) {
