...
export const exportAllData = async () => {
  const [teams, matches, bets] = await Promise.all([
    db.teams.toArray(),
    db.matches.toArray(),
    db.bets.toArray()
  ]);
...
export const importAllData = async (exportedData: any) => {
  ...
  const processTeams = teams?.map((team: any) => ({
    ...team,
    createdAt: new Date(team.createdAt)
  })) || [];

  const processMatches = matches?.map((match: any) => ({
    ...match,
  ...
  await db.transaction('rw', [db.teams, db.matches, db.bets], async () => {
    ...
    if (processTeams.length) await db.teams.bulkAdd(processTeams);
    if (processMatches.length) await db.matches.bulkAdd(processMatches);
    if (processBets.length) await db.bets.bulkAdd(processBets);
  });
};

// Test GitHub API connection
export const testGitHubConnection = async (config: SyncConfig): Promise<boolean> => {
  try {
...
// Save file to GitHub
const saveFileToGitHub = async (config: SyncConfig, data: any, sha?: string) => {
