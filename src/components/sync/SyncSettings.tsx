...
import { SyncConfig, getSyncConfig, setSyncConfig, testGitHubConnection } from '../../services/syncService';
...
export default function SyncSettings({ onClose }: SyncSettingsProps) {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (!success) {
        setError('Connection failed. Please check your token and repository details.');
      }
    } catch (err) {
      setTestResult('error');
        ...
        <div className="space-y-4">
          ...
          {testResult === 'success' && (
            <div className="alert alert-success">
              <span>✓ Connection successful! You can now save the configuration.</span>
            </div>
          )}
          ...
          <div className="form-control">
            ...
            <input
              ...
              onInput={(e) => handleInputChange('githubToken', (e.target as HTMLInputElement).value)}
            />
            <label className="label">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="form-control">
              ...
              <input
                ...
                onInput={(e) => handleInputChange('repoOwner', (e.target as HTMLInputElement).value)}
              />
            </div>
            ...
            <div className="form-control">
              ...
              <input
                ...
                onInput={(e) => handleInputChange('repoName', (e.target as HTMLInputElement).value)}
              />
            </div>
          </div>
          ...
          <div className="form-control">
            ...
            <input
              type="text"
              placeholder="maestro-data.json"
              className="input input-bordered"
              value={config.filePath}
              onInput={(e) => handleInputChange('filePath', (e.target as HTMLInputElement).value)}
            />
            <label className="label">
              <span className="label-text-alt">Path where data will be stored in your repository</span>
            </label>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={handleTest}
              disabled={testing || !config.githubToken || !config.repoOwner || !config.repoName}
            >
